...
socket.on("roomList", function(list) {
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        let HTML = `<li class="room--change" data-icon="${uniCode}" data-rid="${list[i].id}"><i>${uniCode}</i> <div class="room--details">${list[i].name} <div class="room--count">Online: ${list[i].online}</div></div><i class="material-icons roomDelete">delete</i></li>`;
