'use-strict';import{$,$$,hasClass,appendDOM,error,openSettings,operations,settingsInput,prependDOM,returnEmoji}from"/js/clientFunc.js";function getUsers(){$(".users__table").innerHTML="";appendDOM(`<li class="loader__div">
                <div class="lds-roller">
                    ...
                    </li>`;appendDOM(HTML,".users__table",false)}}).catch(()=>{$(".panel--middle .loader__div").innerHTML="";appendDOM("<i class=\"material-icons noselect failed-to-fetch\">warning</i>",".panel--middle .loader__div",false);error("Connection failed")})}const socket=io.connect("/chat",{reconnection:true,reconnectionDelay:1000,reconnectionDelayMax:5000,reconnectionAttempts:10});socket.on("roomList",list=>{for(let r=0;r<$$(".rooms").length;r++){$$(".rooms")[r].innerHTML=""}console.log(list);for(let i=0;i<list.length;i++){const emoji=list[i].icon;const HTML=`<li class="room--change" data-icon="${emoji}" data-rid="${list[i].id}">
                        <i>${emoji}</i> 
                    ...
                    </li>`;appendDOM(HTML,".rooms",false);twemoji.parse($(".rooms"))}appendDOM("<li class=\"room--show\">Show rooms</li>",".rooms",false);if($(".rooms--modal")){for(let i=0;i<list.length;i++){const emoji=returnEmoji(list[i].icon);const HTML=`<li class="room--change" data-icon="${emoji}" data-rid="${list[i].id}">
                            <i>${emoji}</i> 
                            <div class="room--details">${list[i].name} 
                                <div class="room--count">Online: ${list[i].online}</div>
                            </div>
