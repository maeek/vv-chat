'use-strict';import{$,$$,hasClass,appendDOM,error,openSettings,operations,settingsInput,prependDOM}from"/js/clientFunc.js";function getUsers(){$(".users__table").innerHTML="";appendDOM(`<li class="loader__div">
                <div class="lds-roller">
                    ...
                    </li>`;appendDOM(HTML,".users__table",false)}}).catch(()=>{$(".panel--middle .loader__div").innerHTML="";appendDOM("<i class=\"material-icons noselect failed-to-fetch\">warning</i>",".panel--middle .loader__div",false);error("Connection failed")})}const socket=io.connect("/chat",{reconnection:true,reconnectionDelay:1000,reconnectionDelayMax:5000,reconnectionAttempts:10});socket.on("roomList",list=>{for(let r=0;r<$$(".rooms").length;r++){$$(".rooms")[r].innerHTML=""}let roomsList="";for(let i=0;i<list.length;i++){const emoji=list[i].icon;roomsList+=`<li class="room--change" data-icon="${emoji}" data-rid="${list[i].id}">
                    ...
                    </li>\n`}appendDOM(roomsList,".rooms",false);appendDOM("<li class=\"room--show\">Show rooms</li>",".rooms",false);if($(".rooms--modal")){let roomsList="";for(let i=0;i<list.length;i++){const emoji=list[i].icon;roomsList+=`<li class="room--change" data-icon="${emoji}" data-rid="${list[i].id}">
                            <i>${twemoji.parse(emoji)}</i> 
                            <div class="room--details">${list[i].name} 
                                <div class="room--count">Online: ${list[i].online}</div>
                            </div>
                            <i class="material-icons roomDelete">delete</i>
                        </li>\n`}appendDOM(roomsList,".rooms--modal",false)}});window.addEventListener("DOMContentLoaded",()=>{socket.emit("roomList",true);const el=$("aside .info");const wh=document.height!==undefined?document.height:document.body.offsetHeight;const panel=$(".panel--middle");if(location.protocol=="http:"&&!localStorage.getItem("useHTTP")){prependDOM(`<div class="http noselect">
                    ...
                    <i class="material-icons http--close">close</i>
                        ...
                        </div>`;appendDOM(HTML,"body",false);fetch("/js/emoji.json",{method:"GET",headers:{"Content-Type":"application/json"}}).then(res=>res.json()).then(emojis=>{$(".room__icons").innerHTML="";let list="";for(let i=0;i<emojis.length;i++){list+=`<i class="select__icon" data-char="${emojis[i].char}" data-index="${emojis[i].no}">${twemoji.parse(emojis[i].char)}</i>\n`}appendDOM(list,".room__icons",false)}).catch(e=>{console.log(e);$(".room__icons").innerHTML="";appendDOM("<i class=\"material-icons noselect failed-to-fetch\">warning</i>",".modal__div .room__icons",false)});$(".modal__div").classList.add("anim--opacity");$(".settings__cont").classList.add("anim--opacity","anim--scale")}else if(e.target&&hasClass(e.target,"select__icon")||hasClass(e.target.parentNode,"select__icon")){const btn=hasClass(e.target,"select__icon")?e.target:e.target.parentNode;const icons=$$(".select__icon");for(let i=0;i<icons.length;i++){icons[i].classList.remove("icon--active")}btn.classList.add("icon--active");$(".icon--prev").innerHTML=btn.getAttribute("data-char");twemoji.parse($(".icon--prev"))}else if(e.target&&hasClass(e.target,"roomDelete")){e.stopPropagation();const rid=e.target.parentNode.getAttribute("data-rid");socket.emit("deleteRoom",rid)}else if(e.target&&hasClass(e.target,"create--room")||hasClass(e.target.parentNode,"create--room")){const roomName=$("input[name='roomName']").value.trim();const icon=$(".icon--active")?$(".icon--active").getAttribute("data-char"):null;socket.emit("addRoom",{name:roomName,icon:icon},res=>{if(!res.status){error(res.message)}else{$(".settings__cont").classList.remove("anim--opacity","anim--scale");$(".modal__div").classList.remove("anim--opacity");setTimeout(()=>{$(".modal__div").remove()},300)}})}});function block(ids,isChecked){ids=Array.isArray(ids)?ids:[ids];fetch("/manage/",{body:JSON.stringify({action:"block",uids:ids,block:isChecked}),headers:{Accept:"application/json","Content-Type":"application/json"},method:"POST"}).then(res=>res.json()).then(res=>{if(!res.status){for(const el in ids){$(`input[name="block"][value="${el}"]`).checked=!$(`input[name="block"][value="${el}"]`).checked}error("Couldn't block user")}})}document.addEventListener("input",e=>{if(e.target.getAttribute("name")=="roomName"){const format=/^[AaĄąĆćĘęŁłŃńÓóSsŚśŹźŻża-zA-Z0-9@!.-\s]+$/;const val=e.target.value;if(format.test(val)){$(".room--name--prev").innerHTML=val}else if(val.trim().length==0){$(".room--name--prev").innerHTML="Type room name in the field above"}else if(val.trim().length>30){$(".room--name--prev").innerHTML="Maximum length is 30"}else{$(".room--name--prev").innerHTML="Room name can only contain a-z A-Z 0-9 -@!."}}else if(e.target.getAttribute("name")=="blockAll"){const uids=[];if(e.target.checked){for(let i=0;i<$$("input[name=\"block\"]").length;i++){uids.push($$("input[name=\"block\"]")[i].value);$$("input[name=\"block\"]")[i].checked=true}block(uids,true)}else{for(let i=0;i<$$("input[name=\"block\"]").length;i++){uids.push($$("input[name=\"block\"]")[i].value);$$("input[name=\"block\"]")[i].checked=false}block(uids,false)}}else if(e.target.getAttribute("name")=="block"){const clientId=e.target.value;const isChecked=e.target.checked;block(clientId,isChecked)}else if(e.target&&e.target.getAttribute("name")=="searchRoom"){e.preventDefault();let rooms=$$(".room--change");let val=e.target.value.trim().toLowerCase();if(val.length!=0||val!=""){for(let i=0;i<rooms.length;i++){if(rooms[i].querySelector(".room--details").innerText.toLowerCase().replace(/online:\s\d+/g).indexOf(val)==-1){rooms[i].style.display="none"}else{rooms[i].removeAttribute("style")}}}else{for(let i=0;i<rooms.length;i++){rooms[i].removeAttribute("style")}}}});if("serviceWorker"in navigator){navigator.serviceWorker.register("/sw.js",{scope:"/"}).then(r=>{console.log("SW scope:",r.scope);console.log("ServiceWorker zarejestrowany.")}).catch(e=>{console.log(`Ups! Błąd przy rejestracji ServiceWorkera! ${e}`)})}window.addEventListener("DOMContentLoaded",()=>{if($$(".settings--popup").length>0){$(".settings--popup").addEventListener("click",openSettings);document.addEventListener("click",operations,false);document.addEventListener("input",settingsInput,false)}});